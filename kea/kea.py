...
class Kea:
    ...
    def load_properties(cls, property_files):
        ...
        for file in property_files:
            ...
            file_abspath = (
                os.path.join(workspace_path, file) if not os.path.isabs(file) else file
            )
            ...
            module_name, extension_name = [
                str(_) for _ in os.path.splitext(os.path.basename(file_abspath))
            ]
            ...
            try:
                ...
                for _, obj in inspect.getmembers(module):
                    if (
                        inspect.isclass(obj)
                        and issubclass(obj, KeaTest)
                        and obj is not KeaTest
                    ):
    ...
    def init_KeaTestElements(cls, keaTest: "KeaTest") -> "KeaTestElements":
        ...
        current_keaTestElements = cls._KeaTest_DB.get(
            keaTest, KeaTestElements(keaTest_name)
        )
    ...
    def execute_rule(self, rule: "Rule", keaTest: "KeaTest"):
        ...
        try:
            ...
            self.logger.warning(
                f"Error occurred in file {file_name} on line {line_number}:"
            )
